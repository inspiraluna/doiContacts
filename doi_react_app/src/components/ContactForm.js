  ...
  const addContact = async email => {
    const runAddContact = async email => {
      ...
      try {
        ...
        const txData = await createDOIRequestTransaction(
          email,
          our_wallet,
          offChainUtxos
        );
        const encryptedTemplateData = await encryptTemplate(
        ...
        const utxosResponse = await broadcastTransaction(
          txData,
        ...
        const msg =
          "Broadcasted DOI permission transaction to Doichain node...";
        const contact = {
        ...
        <div>
          ...
          >
            <QRCodeScannerTextField
              name="email"
              label={"Email permssion to request"}
              labelWidth={200}
              urlPrefix={"mailto:"}
            />
            ...
            >
              ...
              {wallets.map((wallet, index) => (
                <option key={index} value={index}>
                  {wallet.email} ({wallet.balance} DOI)
                </option>
              ))}
            </NativeSelect>
            <br />
