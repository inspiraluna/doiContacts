...
import s from  './SendAmount.module.css'
...
const SendAmount = () => {
  ...
  const handleCancel = e => {
    ...
    window.QRScanner.destroy(destroyStatus => {
      ...
      if (destroyStatus.scanning || destroyStatus.previewing)
        window.QRScanner.cancelScan(function(cancelStatus) {
  ...
  const handleSendTransaction = (toAddress, amount) => {
    ...
    try {
      const ourAddress =
        global.wallets[global.activeWallet].addresses[0].address;
      ...
      bitcore.getUTXOAndBalance(ourAddress, amount).then(function(utxo) {
        if (
          utxo.utxos.length === 0 &&
          (!global.utxos || global.utxos.length === 0)
        ) {
        ...
        bitcore
          .broadcastTransaction(null, txSerialized, null, null)
          .then(response => {
  ...
  function showScanner() {
    console.log(
      "showing scanner - cordova available",
  ...
  function scan() {
    ...
    function displayContents(err, text) {
      if (err) {
        ...
        window.QRScanner.getStatus(function(status) {
          console.log(
            "error QRScanner status: err" + err,
        ...
        window.QRScanner.getStatus(function(status) {
          console.log(
            "QRScanner success status: text" + text,
            JSON.stringify(status)
          );

          const result = text.result === undefined ? text : text.result;
          if (result.startsWith("doicoin:")) setToAddress(result.substring(8));
          else console.log("different qr code stopping scan");
        });
      }
      handleCancel();
    }
          ...
          {({
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div>
                {" "}
                <FormControl fullWidth variant="outlined">
                  <InputLabel htmlFor="outlined-adornment-passworoutlined-adornment-amount">
                    Amount (DOI)
                  </InputLabel>
                  <OutlinedInput
                    id="amount"
