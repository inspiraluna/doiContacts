...
class QRCodeScannerContents extends React.Component {
    render() {
        if (this.props.scanning) return <QRCodeScannerStopButton />
        else {
...
export const QRCodeScannerTextField = ({
    label,
    labelWidth,
    urlPrefix,
    name,
    ...
    const setScanning = useGlobal("scanning")[1]
    ...
    function scan() {
        ...
        function displayContents(err, text) {
            if (err) {
                ...
                window.QRScanner.getStatus(function(status) {
                    console.log(
                        "QRScanner success status: text:\n" +
                            JSON.stringify(status),
                        text
                    )

                    const result =
                        text.result === undefined ? text : text.result
                    if (
                        urlPrefix.length === 0 ||
                        urlPrefix === undefined ||
                        result.startsWith(urlPrefix)
                    )
                        setQRCode(result.substring(urlPrefix.length))
                    else
                        console.log(
                            "stopping scan because qr code incompatible with form, url should start with ",
                            urlPrefix
                        )
                })
            }
