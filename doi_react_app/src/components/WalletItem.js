    ...
    if (publicKey && !balance) {
      ...
      fetchBalanceData(generatedAddress).then(retBalanceData => {
        if (retBalanceData) {
          ...
          if (retBalanceData.balance == balance) setUnconfirmedBalance(0)
          //reset unconfirmedBalance in case balance from Node is the same as here
          else setUnconfirmedBalance(retBalanceData.unconfirmedBalance)
        }
      })
    }
  }, [balance])
    ...
    return (
      <div>
        <li style={{ fontSize: "9px" }}>
          DoiCoin-Address: <br />
          <b>
            {address ? address.toString() : ""} <CopyToClipboard text={address ? address.toString() : ""}
          onCopy={() => setOpenSnackbar({open:true,msg:'copied',type:'success'})}>
        ...
        <div style={{ fontSize: "9px", border: "2px solid lightgrey" }}>
          ...
          <br />
          <label htmlFor={"contentType"}></label>Content-Type: {contentType}
          <br />
          <label htmlFor={"redirectUrl"}></label>Redirect-Url: {redirectUrl}>
          <br />
